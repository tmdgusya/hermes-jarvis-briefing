...
- `/jarvis demo` 입력 → 박수 두 번 후 화면이 **JARVIS ON** 으로 켜지고, #바이브코딩 Threads 인기글 데모 대화를 기다림
...
2. `/jarvis demo` 입력
3. 준비 비프 후 박수 2번
4. 웹 오버레이가 `JARVIS ON` 으로 전환되면 아래처럼 말하기
   - `스레드에서 72시간 동안 #바이브코딩 키워드로 발행된 인기글 브리핑해줘`
5. Jarvis가 고정 Top 3를 빠르게 응답
6. 이어서 `공통점이 있을까?` 라고 물으면 공통점 요약 응답

데모 모드는 다음 사용자 발화부터 적용되도록 세션 히스토리에 비공개 프라이머를 심고, `/jarvis demo` 직후에는 별도 LLM 턴을 시작하지 않습니다. 따라서 화면에는 `JARVIS ON` 만 뜨고 자연스럽게 사용자의 음성 질문을 기다립니다.
