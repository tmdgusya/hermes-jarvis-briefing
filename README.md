...
- `/jarvis demo` 입력 → 박수 두 번 후 화면이 **JARVIS ON** 으로 켜지고, #바이브코딩 Threads 인기글을 3~5턴 정도 대화하는 데모 모드 시작
- `/jarvis` 입력 시 데모 웹뷰 서버와 브라우저 오버레이도 자동 기동 (최초 1회)
- 웹 오버레이는 브라우저 마이크 입력을 읽어 말하는 음성에 맞춰 원형 파형/물결을 일렁이게 표시
- 준비 비프 → 박수 두 번 (3초 이내) → 확인 비프 → 브리핑 시작
- LLM 이 `weather` 스킬을 호출하고 오늘의 할 일을 결합해 **3문장 한국어 브리핑**을 생성
- TTS 로 자동 낭독
- 브리핑 끝나면 **마이크가 자동으로 열립니다** — Ctrl+B 를 누를 필요 없이 바로 말하면 되고, 3초 침묵이면 자동 전송 (continuous voice mode). Ctrl+B 를 눌러 종료 가능.
...
# 세션 안에서:
/jarvis              # 오늘 (기본)
/jarvis today        # 명시적으로 오늘
/jarvis tomorrow     # 내일
/jarvis week         # 이번주 (월~일)
/jarvis demo         # 촬영용: 박수 2번 → JARVIS ON → Threads #바이브코딩 데모 대화 대기
/jarvis threads      # demo 별칭
```

- 세션 콘솔에 "Jarvis listening. 박수 두 번으로 브리핑 시작 (30초 타임아웃)." 표시
- 웹뷰 오버레이가 자동으로 열리면, 그 화면에서 상태(HUD)를 함께 시연 가능
- 짧은 비프 한 번이 들리면 준비 완료 — **박수 두 번 (0.3초~3초 간격)**
- 확인 비프 두 번 + 브리핑 TTS
- 브리핑이 끝나면 mic 가 자동으로 열림 → 바로 답하거나 후속 질문 가능. 3초 침묵 = 자동 전송. 대화를 끝내려면 Ctrl+B.

### Threads #바이브코딩 데모 촬영 플로우

자세한 촬영 순서, 대본, 실패 시 대처법은 [`DEMO.md`](./DEMO.md)에 정리되어 있습니다.

실제 Threads 수집 API나 웹 자동화를 기다리면 촬영 중 속도와 결과가 흔들릴 수 있어서, `/jarvis demo` 는 의도적으로 **스크립트 기반 데모 모드**로 동작합니다.

1. `hermes chat` 실행
2. `/jarvis demo` 입력
3. 준비 비프 후 박수 2번
4. 웹 오버레이가 `JARVIS ON` 으로 전환되고 파형이 움직이면 아래처럼 말하기
   - `스레드에서 72시간 동안 #바이브코딩 키워드로 발행된 인기글 브리핑해줘`
5. Jarvis가 먼저 `반응이 좋은 글이 3건 있습니다. 바로 읽어드릴까요?` 라고 제안
6. `응, 읽어줘.` 라고 허락하면 Jarvis가 고정 Top 3를 빠르게 응답
7. 이어서 아래처럼 3~4턴 더 대화
   - `공통점이 있을까?`
   - `그러면 내가 오늘 올릴 만한 건 뭐가 좋을까?`
   - `하나만 골라줘.`
   - `그걸 캡션까지 써줘.`

데모 모드는 다음 사용자 발화부터 적용되도록 세션 히스토리에 비공개 프라이머를 심고, `/jarvis demo` 직후에는 별도 LLM 턴을 시작하지 않습니다. 박수 후에는 `JARVIS ON` 상태가 뜨고 곧바로 음성 녹음이 열리며, 이후 대화 턴마다 오버레이가 `듣는 중 → 생성 중 → 말하는 중` 상태로 바뀝니다.

### 범위별 브리핑 성격

| 인자 | 관점 | 리드 문장 | 형식 |
|---|---|---|---|
| `today` / (없음) | 하루 집중 | 오늘 가장 놓치면 안 될 한 건 | 시간만 `HH:MM` |
| `tomorrow` | 내일 대비 | 내일 가장 놓치면 안 될 한 건 | 시간만 `HH:MM` |
| `week` | 주간 조망 | 이번주 가장 주목해야 할 이벤트 | `M/D (요일) HH:MM` |

주간 브리핑은 프롬프트 지침 자체가 달라져서 "뉴스 앵커" 톤이 아닌 **"주간 기획 회의 리더"** 톤으로 나옵니다. 요일별 흐름 요약이 3문장째에 자연스럽게 들어갑니다.
